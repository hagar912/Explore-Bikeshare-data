...
##import dataset
getwd()
setwd('I:/NTL/AAL/R course/final_project/chi')
...
##making Age.Class as anew_col for classify the Birth.Year as 3 levels (Young ,Youth ,Child)
summary(chi$Birth.Year)
Age.Class <- ifelse(
...
start_time <-
    sapply(strsplit(as.character(chi$Start.Time), " "), "[", 2)
...
#split the date
start_date <-
    sapply(strsplit(as.character(chi$Start.Time), " "), "[", 1)
##suset month from the data
month <- substr(x = start_date, 6, 7)
chi['month'] <- month
unique(chi$month, incomparables = FALSE)
chi$month[chi$month == "01"] <- 'January'
chi$month[chi$month == "02"] <- 'February'
chi$month[chi$month == "03"] <- 'March'
chi$month[chi$month == "04"] <- 'April'
chi$month[chi$month == "05"] <- 'May'
chi$month[chi$month == "06"] <- 'June'
...
rental_gender(subset(chi,!is.na(Gender)))
dev.off()
...
rental_age_stage(subset(chi,!is.na(Age.Class)))
dev.off()

#..................... From Plot(1,2,3)  .........................#
# ........ We found that Subscriber user who is Male with from 41 and above was the best use for the bike rental ............#
# ................................................................................................................................#

# .. 4 .. #
#Q(4) .. Comparing the growth of the usage with full time for the trip between the months !
## from the plot , we considered that , There is a growth for the usage of the bike during months as : During April the median was 697 and in may was 709 and then there is a great growth in juna as it was 772
...
growth_months(data = subset(chi, month == c('June', 'May' , 'April')))
dev.off()
...
duration_gender(subset(chi, !is.na(Gender)))
dev.off()
